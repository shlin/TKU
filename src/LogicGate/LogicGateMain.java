...
public class LogicGateMain {
	...
	public static void main(String[] args) throws FileNotFoundException,
		...
		while (inputItor.hasNext()) {
			...
			Iterator<File> inputFileItor = newInput.getInputMap().values()
					.iterator();
			while (inputFileItor.hasNext()) {
				File srcFile = inputFileItor.next();
				File dstFile = new File(dirOutput.getAbsolutePath() + "/"
						+ srcFile.getName().replace("ip", "out"));
				ProcInput loader = new ProcInput( gateLists, srcFile, dstFile,
						benchFiles.get(newInput.getBenchID()));
